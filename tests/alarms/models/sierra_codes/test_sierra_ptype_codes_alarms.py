...
class TestSierraPtypeCodesAlarms:
    @pytest.fixture
    def test_instance(self, mocker):
        mock_redshift_client = mocker.MagicMock()
        mock_sierra_client = mocker.MagicMock()
        return SierraPtypeCodesAlarms(mock_redshift_client, mock_sierra_client)

    def test_init(self, mocker):
        mock_redshift_client = mocker.MagicMock()
        mock_sierra_client = mocker.MagicMock()
        sierra_ptype_codes_alarms = SierraPtypeCodesAlarms(
            mock_redshift_client, mock_sierra_client
        )
        assert sierra_ptype_codes_alarms.redshift_suffix == "_test_redshift_db"
        assert sierra_ptype_codes_alarms.run_added_tests
        assert sierra_ptype_codes_alarms.yesterday_date == date(2023, 5, 31)
        assert sierra_ptype_codes_alarms.yesterday == "2023-05-31"

    def test_run_checks_no_alarms(self, test_instance, mocker, caplog):
        mock_sierra_query = mocker.patch(
            "alarms.models.sierra_codes.sierra_ptype_codes_alarms.build_sierra_ptypes_count_query",
            return_value="sierra ptypes query",
        )
        mock_redshift_counts_query = mocker.patch(
            "alarms.models.sierra_codes.sierra_ptype_codes_alarms.build_redshift_code_counts_query",
            return_value="redshift code counts query",
        )
        mock_redshift_null_query = mocker.patch(
            "alarms.models.sierra_codes.sierra_ptype_codes_alarms.build_redshift_ptype_null_query",
        ...
        test_instance.sierra_client.execute_query.assert_has_calls(
            [mocker.call("sierra ptypes query")]
        )
        ...
        mock_redshift_counts_query.assert_called_once_with(
            "code", "sierra_ptype_codes_test_redshift_db"
        )
        mock_redshift_null_query.assert_called_once_with(
            "sierra_ptype_codes_test_redshift_db", "2023-05-31"
        )
    ...
    def test_run_checks_unequal_counts_alarm(self, test_instance, mocker, caplog):
        ...
        with caplog.at_level(logging.ERROR):
            test_instance.run_checks()
        assert (
            "Number of Sierra ptype records does not match number of "
            "Redshift ptype records: 10 Sierra ptype records and 20 Redshift records"
        ) in caplog.text
    ...
    def test_run_checks_duplicate_codes_alarm(self, test_instance, mocker, caplog):
        ...
        with caplog.at_level(logging.ERROR):
            test_instance.run_checks()
        assert (
            "Duplicate ptype codes found in Redshift: 10 total active "
            "ptype codes but only 9 distinct active ptype codes"
        ) in caplog.text
    ...
    def test_run_checks_alarms_null_fields_alarm(self, test_instance, mocker, caplog):
        ...
        with caplog.at_level(logging.ERROR):
            test_instance.run_checks()
        assert (
            "The following ptype_codes have a null value for one or more of their "
