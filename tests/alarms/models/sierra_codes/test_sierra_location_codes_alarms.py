...
class TestSierraLocationCodesAlarms:
    ...
    def test_instance(self, mocker):
        test_logger = logging.getLogger("test")     # Custom logger since Alarm types use AlarmController logger
        mock_redshift_client = mocker.MagicMock()
        mock_sierra_client = mocker.MagicMock()
        return SierraLocationCodesAlarms(test_logger, mock_redshift_client, 
                               mock_sierra_client)
    ...
    def test_init(self, mocker):
       ...
       mock_redshift_client = mocker.MagicMock()
       mock_sierra_client = mocker.MagicMock()
       sierra_location_codes_alarms = SierraLocationCodesAlarms(mock_logger, mock_redshift_client, 
                                           mock_sierra_client)
    ...
    def test_run_checks_unequal_counts_alarm(
            self, test_instance, mocker, caplog):
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_sierra_code_count_query')
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_redshift_code_counts_query')
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_redshift_location_null_query')
        test_instance.sierra_client.execute_query.return_value = [(10,)]
        test_instance.redshift_client.execute_query.side_effect = [
            ([20, 20],), ()]
    ...
    def test_run_checks_duplicate_codes_alarm(
            self, test_instance, mocker, caplog):
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_sierra_code_count_query')
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_redshift_code_counts_query')
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_redshift_location_null_query')
    ...
    def test_run_checks_null_fields_alarm(
            self, test_instance, mocker, caplog):
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_sierra_code_count_query')
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_redshift_code_counts_query')
        mocker.patch('alarms.models.sierra_codes.sierra_location_codes_alarms.build_redshift_location_null_query')
        ...
        with caplog.at_level(logging.ERROR):
            test_instance.run_checks()
        assert ("The following location_codes have a null value for one or more of "
