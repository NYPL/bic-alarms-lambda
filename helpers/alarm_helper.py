...
def redshift_mismatch_alarm(logger, database_type, redshift_table,
...
def no_records_found_alarm(logger, database_count, conditional,
                               database_type, date):
...
def sierra_null_codes_alarm(logger, null_codes, code_type):
    if len(null_codes) > 0:
        null_codes_log = _SIERRA_NULL_CODES_LOG.format(code_type=code_type, 
                                                       codes=null_codes)
        logger.error(null_codes_log)
        

def sierra_duplicate_code_alarm(logger, code_type, total_count, 
                                distinct_count):
    if total_count != distinct_count:
        duplicate_code_log = _SIERRA_DUPLICATE_CODE_LOG.format(code_type=code_type, 
